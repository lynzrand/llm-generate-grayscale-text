...
# Since the tokenizer might tokenize one character into multiple tokens,
# we create a trie to store the average grayscale of each token.


class TokenGrayscale:
    value: float
    is_end_of_string: bool
    children: dict[int, "TokenGrayscale"]
    leaf_count: int
    ...
    def add(self, token_string: list[int], value: float, idx=0):
        "Add a value associated with a token string to the trie."
        self.leaf_count += 1

        if idx == len(token_string):
            self.value = value
        ...
        if token not in self.children:
            self.children[token] = TokenGrayscale()
        self.children[token].add(token_string, value, idx + 1)

    def calc_avg_grayscale(self):
        "Calculate the average grayscale value for all children of this node"
        if not self.children:
            return self.value

        total = 0
        for child in self.children.values():
            total += child.calc_avg_grayscale() * child.leaf_count
        self.value = total / self.leaf_count
        return self.value

    def query(self, token_string: list[int], idx=0) -> float | None:
        "Query the value associated with a token string in the trie. Returns None if not found."
        if idx == len(token_string):
            # Note that this also returns for non-leaf nodes
            return self.value

        token = token_string[idx]
        if token not in self.children:
            return None
        return self.children[token].query(token_string, idx + 1)

    def is_token_string_complete(self, token_string: list[int], idx=0) -> bool | None:
        "Check if the token string can be considered complete"
        if idx == len(token_string):
        ...
        if token not in self.children:
            return None
        return self.children[token].is_token_string_complete(token_string, idx + 1)

    def __str__(self, level=0):
        """
        Return a string representation of the TokenGrayscale object as an indented tree.
        """
        indent = "  " * level
        result = f"{indent}Value: {self.value}, Leaf Count: {self.leaf_count}, End: {self.is_end_of_string}\n"
        for token, child in self.children.items():
            result += f"{indent}  Token: {token}\n"
